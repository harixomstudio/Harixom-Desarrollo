...
export default function Profile(props: ProfileProps) {
  ...
  const [services, setServices] = React.useState("Fnasarts\nSibek\nMecha\nMega\nCore");
  const [prices, setPrices] = React.useState("Chibi: $10\nHalf body: $15\nFull body: $25");
  const [terms, setTerms] = React.useState("El pago completo se hace primero.\nUna vez realizado el dibujo se empezará.\nNo se aceptan reembolsos.");
  ...
  const [comments, setComments] = React.useState([
    {
      user: "usernameanon",
      message: "I have no idea how to bookmark, click circle to something idk. Right...",
    },
  ]);
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-stone-800 rounded-lg p-6 text-gray-200">
                  <h3 className="text-pink-400 font-bold text-lg mb-4">Services</h3>
                  {editing ? (
                    <textarea
                      value={services}
                    ...
                    <ul className="space-y-2 text-sm whitespace-pre-line">{services}</ul>
                  )}
                </div>
                ...
                <div className="bg-stone-800 rounded-lg p-6 text-gray-200">
                  ...
                  {editing ? (
                    ...
                    <ul className="space-y-2 text-sm whitespace-pre-line">{prices}</ul>
                  )}
                </div>

                {/* Términos */}
                <div className="bg-stone-800 rounded-lg p-6 text-gray-200">
                  <h3 className="text-pink-400 font-bold text-lg mb-4">Terms and Conditions</h3>
                  {editing ? (
                    <textarea
                      value={terms}
                    ...
                    <ul className="space-y-2 text-sm whitespace-pre-line">{terms}</ul>
                  )}
                </div>
              </div>
            </>
            ...
            <div className="w-full max-w-4xl mx-auto">
              ...
              <div className="mb-6">
                ...
                <div className="flex justify-end mt-2">
                  <button
                    onClick={() => {
                      if (newComment.trim()) {
                        setComments([{ user: props.username, message: newComment }, ...comments]);
                        setNewComment("");
                      }
                    }}
                    className="bg-pink-500 hover:bg-pink-600 text-white font-semibold px-4 py-2 rounded-full transition-all"
              ...
              <div className="space-y-4">
                {comments.map((comment, i) => (
                  <div key={i} className="bg-stone-800 p-4 rounded-lg text-gray-200">
                    <p className="text-sm font-semibold text-pink-400">{comment.user}</p>
