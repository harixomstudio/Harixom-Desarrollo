...
export default function FeedPage({ publications }: FeedPageProps) {
  ...
  useEffect(() => {
    ...
    const initialLikes = publications.reduce(
      (acc, pub) => {
  ...
  `}
      ...
      {visibleCount < publications.length ? ( // esto es el loading se activa al scrollear
        ...
        <div className="flex space-x-1 items-baseline text-gray-400 text-sm text-center justify-center pb-5 pt-10">
          {" "}
          WAITING FOR NEW POSTS
          <p className="animate-pulse [animation-delay:-0.8s] text-2xl pl-1">
            .
          </p>
