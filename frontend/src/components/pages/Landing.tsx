...
export default function Landing(props: LandingProps) {
  ...
  return (
    <main className="pt-40 w-full bg-[#141414] text-white ">
      {/* PayPalApi with label for amount 
                <label htmlFor="amount" className=" text-4xl font-berkshire justify-center"  >Comission: $ </label> 
                <input className=" font-berkshire text-4xl py-2 border-b-2 border-white " type="number"  id="amount" min="0.00" step="1" placeholder={amount === 0 ? "Digite la comision" : "0"} value={amount === 0 ? "0" : amount} onChange={(e) => setAmount(parseInt(e.target.value))} />
                <div className={`${amount === 0 ? "opacity-0 scale-y-0" : ""} pt-5 transform transition duration-700 delay-500`} ><PayPalApi value={amount} /></div> 
            */}
      ...
      <section className="flex flex-col items-center justify-center gap-20 py-75 max-lg:gap-10 max-lg:py-30 px-15 max-lg:px-5">
        <h2 className="text-7xl  max-lg:text-4xl pb-15 max-lg:pb-10 font-berkshire">
          {" "}
          Categories
        </h2>
          ...
          {props.categoriesUp.map((categoriesUp, number) => (
            <Link to={props.links[number]}>
              <img
                src={categoriesUp}
                alt={`Image ${number}`}
                className="w-30 h-30 max-xl:w-22 max-xl:h-22 duration-600 max-lg:w-15 max-lg:h-15 hover:scale-110 hover:shadow-lg shadow-black"
              />
            </Link>
          ))}
        </div>
          ...
          {props.categoriesDown.map((categoriesDown, number) => (
            <Link to={props.links2[number]}>
              <img
                src={categoriesDown}
                alt={`Image ${number}`}
                className="w-30 h-30 duration-600 max-xl:w-22 max-xl:h-22 max-lg:w-15 max-lg:h-15 hover:scale-110 hover:shadow-lg shadow-black"
              />
            </Link>
          ))}
        </div>
      ...
      <section className="flex items-center py-30 bg-[#FFAFEE] text-black px-15 gap-35 max-lg:flex-col max-lg:px-5">
        ...
        <div className="flex flex-col w-1/2 text-justify justify-center gap-35 max-lg:gap-5 max-lg:w-full max-lg:items-center">
          <h2 className="text-5xl max-lg:text-3xl font-berkshire">
            {" "}
      ...
      <section className="flex flex-col items-center justify-center gap-25 py-50 max-lg:gap-10 max-lg:py-30 ">
        <h2 className="text-7xl max-lg:text-5xl pb-20 max-lg:pb-10 font-berkshire">
          Artists ranking
        </h2>
        <div className="flex flex-wrap justify-between w-full gap-30 max-lg:gap-10 text-2xl max-lg:text-sm px-15 max-lg:px-5">
          {props.rankingArtist.map((rankingArtist, number) => (
            <Link
              ...
              className="flex flex-col items-center w-3/7 hover:scale-110 duration-600 text-center gap-10 max-lg:gap-3 max-lg:pb-10 overflow-hidden"
            >
              <img
                src={props.rankImg[number]}
                alt={`Image ${number}`}
                className="bg-amber-50 w-20 h-20 max-lg:w-10 max-lg:h-10 rounded-full "
              />

              <img
                src={rankingArtist}
                alt={`Image ${number}`}
                className="bg-amber-50 w-full h-90 max-lg:h-50 object-cover rounded-4xl duration-600 hover:shadow-lg shadow-black"
              />
      ...
      <section className="flex flex-col relative justify-center text-center bg-[#A39FF6] max-lg:h-1/1 text-black w-full h-screen py-30 ">
        <h2 className="text-8xl max-lg:text-4xl pb-40 max-lg:pb-15 font-berkshire">
          Commisions
        </h2>
        <div className=" flex flex-wrap w-full text-2xl max-lg:text-lg max-lg:px-5">
          {props.commisionsCategories.map((commisionsCategories, number) => (
            <Link
              to={props.linksCommisions[number]}
              className="flex flex-col w-1/3 pb-10 cursor-default max-lg:w-1/2 "
            >
              <p className="font-semibold w-auto mx-auto hover:scale-110 duration-600 hover:text-[#FA6063] cursor-pointer ">
                {commisionsCategories}
              </p>
            </Link>
          ))}
        </div>
        ...
        <img
          src="lineDecoration.svg"
          alt="decoration"
          className="absolute w-full bottom-0 translate-y-1/2"
        />
      </section>
